...
export const getLoginFormData = async (): Promise<IAttributes[]> => {
  try {
    ...
    // attributes may come as a different internal type; cast via unknown to satisfy TypeScript
...
export const handleLoginSubmit = async (inputValues: {
  ...
  try {
    ...
    // narrow response type: ensure it's an auth entity with userIdentifier
    const isAuthEntity = (r: any): r is { userIdentifier: string; accessToken?: string; refreshToken?: string } =>
      !!r && typeof r === 'object' && 'userIdentifier' in r;

    if (!isAuthEntity(response)) {
